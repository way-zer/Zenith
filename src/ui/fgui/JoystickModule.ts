...
export class JoystickModule extends egret.EventDispatcher {
    ...
    private OnTouchMove(evt: egret.TouchEvent): void {
        if(this.touchId != -1 && evt.touchPointID == this.touchId) {
            var bx: number = evt.stageX / fgui.GRoot.contentScaleFactor;
            var by: number = evt.stageY / fgui.GRoot.contentScaleFactor;
